...
type
 ...
 http://edgarpavao.com/2017/08/07/multithreading-e-processamento-paralelo-no-delphi-ppl/
  ...
  private
    ...
    StrTemp: string;
  public
    constructor Create(mymemo: tmemo; cargs: TStringList; loadbar: TProgressBar);
      reintroduce;
...
begin
  ...
  if (type_str = 4) then
    ...
    Self.args.AddStrings(Self.str_args)
...
begin
  ...
  case self.RadioGroup1.ItemIndex of
    ...
    begin
      Self.str_args := '--i-d';
      Self.str_args :=
        Self.str_args + ' ' + InputBox('Install', 'Entre com a lista de pacotes',
